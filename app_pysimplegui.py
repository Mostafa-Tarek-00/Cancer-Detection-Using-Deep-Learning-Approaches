...
# Save masks as .nii.gz file
def save_masks(cropped_img, segmented_img, save_path):
    cropped_img.save(save_path + '/cropped_image.jpg')
    segmented_img = Image.fromarray(segmented_img)
    segmented_img.save(save_path + '/segmented_image.jpg')
...
def main():
    sg.theme('LightGrey1')
    # layout = [
    #     [sg.Text('Upload Image:'), sg.InputText(key='-FILE-'), sg.FileBrowse()],
    #     [sg.Button('Segment and Classify')],
    #     [sg.Text(size=(80, 1), key='-OUTPUT-')],
    #     [sg.Image(key='-ORIGINAL-')],
    #     # [sg.Image(key='-SEGMENTED-')],
    #     [sg.Image(key='-CROPPED-')],
    #     [sg.Image(key='-LOCALIZED-')]
    # ]
    ...
    [sg.Text(size=(80, 1), key='-OUTPUT-')],
    # [sg.Image(key='-SEGMENTED-')],
    [sg.Image(key='-ORIGINAL-', size=(200, 200)), sg.Image(key='-CROPPED-', size=(200, 200)), sg.Image(key='-LOCALIZED-', size=(200, 200))]
    ]

    window = sg.Window('Skin Lesion Classifier', layout)

    while True:
        event, values = window.read()
        if event == sg.WINDOW_CLOSED:
            break
        if event == 'Segment and Classify':
            filename = values['-FILE-']
            if filename:
                img = Image.open(filename)
                orig_img, img_pred, cropped_img, segmented_img = segment_image(img)

                # Display the prediction text
                lesion_type, confidence = predict(cropped_img)
                output_text = f"Lesion Type: {lesion_type}\nConfidence: {confidence:.2f}"
                window['-OUTPUT-'].update(output_text)

                # Display the original image
                bio = io.BytesIO()
                orig_img.save(bio, format="PNG")
                window['-ORIGINAL-'].update(data=bio.getvalue())

                # Display the segmented image
                bio = io.BytesIO()
                LOCALIZED_img_pil = Image.fromarray(segmented_img)
                LOCALIZED_img_pil.save(bio, format="PNG")
                window['-LOCALIZED-'].update(data=bio.getvalue())

                # Display the cropped image
                bio = io.BytesIO()
                cropped_img.save(bio, format="PNG")
                window['-CROPPED-'].update(data=bio.getvalue())
